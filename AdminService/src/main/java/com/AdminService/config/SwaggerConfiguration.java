...
public class SwaggerConfiguration {
	...
	public Docket swaggerConfiguration1() {
		return new Docket(DocumentationType.SWAGGER_2).select().paths(PathSelectors.any())
				.apis(RequestHandlerSelectors.basePackage("caseStudy.fareMicroservice")).build().apiInfo(apiDetails());

	}

	private ApiInfo apiDetails() {
		return new ApiInfo("Fare-MicroService API Documentation", "API for Fare-MicroService", "1.0", "Free to use",
